...
exports.addStudent = async (req, res) => {
  ...
  if (!nameValidator.test(name)) {
    return res.status(400).json({
      error:
        'Name should have at least 5 characters and should not any number!',
    });
  }
...
exports.feeSection = async (req, res) => {
  try {
    const status = await Application.find({ status: 'Approved' })
...
exports.Library = async (req, res) => {
  try {
    const status = await Application.find({ status: 'Library' })
...
exports.inchargeCampus = async (req, res) => {
  try {
    const status = await Application.find({ status: 'Incharge Campus' })
      .select('-status -phoneNo -description')
      .sort('-_id');
    res.send(status);
  } catch (e) {
